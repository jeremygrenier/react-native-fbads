...
class NativeAdsManager {
    ...
    /** {@number} of unique native ads that can be accessed **/
    uniqueNativeAdCount: number;
    ...
    _listenForStateChanges() {
        NativeAppEventEmitter.addListener("CTKNativeAdsManagersChanged", managers => {
            const values = managers[this.placementId];
            const isValidNew = values.isValid;
            ...
            if (isValid !== isValidNew) {
                ...
                this.uniqueNativeAdCount = values.uniqueNativeAdCount;
            }
        });
    }
